...
import (
	...
	"github.com/openshift/origin/pkg/dockerregistry/server/limit"
)
...
func Execute(configFile io.Reader) {
	...
	if extraConfig.MaxWriters > 0 {
		ctx = server.WithBlobStoreFactory(ctx, limit.NewBlobStoreFactory(extraConfig.MaxWriters))
	}
